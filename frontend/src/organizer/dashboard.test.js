...
test('loadOrganizerDashboard passes auth token and normalizes metrics', async () => {
    ...
    getOrganizerDashboard: async (token) => {
      ...
      return {
        data: {
          ...
          campaigns: [{ id: 7, name: 'کمپین تست', channel: 'email', audience_type: 'confirmed_only', status: 'draft', recipients_count: 10, sent_count: 0, event: { id: 10, title: 'رویداد تست', slug: 'test-event' } }],
  ...
  assert.equal(dashboard.campaigns[0].event.href, '/events/test-event')
})
